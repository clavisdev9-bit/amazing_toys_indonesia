...
async function syncProducts() {
  ...
  for (const op of odooProducts) {
    ...
    if (!op.barcode) {
      ...
      audit.log({ operation_type: 'PRODUCT_SYNC', entity_type: 'product', odoo_entity_id: op.id, action: 'SKIP', status: 'SUCCESS', request_summary: `No barcode: ${op.name}` });
      continue;
    }
    ...
    try {
      ...
      if (!sosProduct) {
        ...
        await xref.upsertXref('product', String(op.id), null, { sos_product_id: sosId, barcode: op.barcode });
        stats.created++;
        audit.log({ operation_type: 'PRODUCT_SYNC', entity_type: 'product', sos_entity_id: sosId, odoo_entity_id: op.id, action: 'CREATE', status: 'SUCCESS' });
      } else {
        ...
        if (changed) {
          ...
          stats.updated++;
          audit.log({ operation_type: 'PRODUCT_SYNC', entity_type: 'product', sos_entity_id: String(sosId), odoo_entity_id: op.id, action: 'UPDATE', status: 'SUCCESS' });
        } else {
          stats.skipped++;
          audit.log({ operation_type: 'PRODUCT_SYNC', entity_type: 'product', sos_entity_id: String(sosId), odoo_entity_id: op.id, action: 'SKIP', status: 'SUCCESS' });
        }
        await xref.upsertXref('product', String(op.id), null, { sos_product_id: String(sosId), barcode: op.barcode });
      }
    } catch (err) {
      stats.failed++;
      logger.error('Product sync: product failed', { odoo_id: op.id, name: op.name, error: err.message });
      audit.log({ operation_type: 'PRODUCT_SYNC', entity_type: 'product', odoo_entity_id: op.id, action: 'FAIL', status: 'FAILED', error_message: err.message });
    }
  }
