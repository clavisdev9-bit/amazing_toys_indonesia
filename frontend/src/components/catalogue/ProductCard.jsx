...
export default function ProductCard({ product, tourAttr, isFirstCard }) {
  ...
  const [bouncing, setBouncing] = useState(false);
  const [imgError, setImgError] = useState(false);
  const [imgLoaded, setImgLoaded] = useState(false); // ← TAMBAHAN: track apakah gambar sudah load
  const config       = usePublicConfig();
  const isHelperMode  = (config?.order_mode ?? 'HELPER_INPUT') === 'HELPER_INPUT';
  const isApproveMode = config?.order_mode === 'HELPER_APPROVE';
      ...
      >
        {product.image_url && !imgError
          ? <img
              ...
              alt={product.name}
              loading="lazy"          // ← TAMBAHAN: native browser lazy load
              decoding="async"        // ← TAMBAHAN: decode gambar di background, tidak block UI
              className="w-full h-full object-contain transition-opacity duration-300"
              style={{ opacity: imgLoaded ? 1 : 0 }} // ← TAMBAHAN: fade-in saat gambar siap
        ...
        {/* TAMBAHAN: Shimmer skeleton selama gambar belum selesai load */}
        {product.image_url && !imgError && !imgLoaded && (
          <div
            className="absolute inset-0 animate-pulse"
            style={{ background: 'rgba(200,210,240,0.35)' }}
          />
        )}
