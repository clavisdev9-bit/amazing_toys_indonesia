...
import { getPreorderList, updateShipment, confirmArrived } from '../../api/preorder';
...
const STATUS_TABS = [
  ...
  { key: 'shipped',   label: 'Dalam Pengiriman',      color: '#2563EB' },
    ...
    <div style={{ background: '#fff', borderRadius: 12, border: `1px solid ${C.border}`, padding: '12px 14px', marginBottom: 10 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
        <div style={{ minWidth: 0, flex: 1, paddingRight: 8 }}>
          ...
          <p style={{ margin: '2px 0 0', fontSize: 11.5, color: C.muted }}>{formatDate(txn.paid_at)}</p>
        </div>
      ...
      {/* Action buttons */}
      {tabKey === 'awaiting' && (
        <button onClick={() => onAction('ship', txn)}
          style={{ width: '100%', padding: '8px 0', borderRadius: 9, background: '#2563EB', color: '#fff', border: 'none', fontWeight: 800, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>
          📦 Input Resi & Kirim
        </button>
      )}
      {tabKey === 'shipped' && (
